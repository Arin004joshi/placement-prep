...
const Practice = () => {
  ...
  return (
    <div className="space-y-5">
      <section>
        <h1 className="text-2xl font-bold text-ink dark:text-slate-100">Practice Mode</h1>
      ...
      <section className="rounded-md border border-line bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
        ...
        <h2 className="text-xl font-semibold text-ink dark:text-slate-100">{currentQuestion.title || "Practice question"}</h2>
        <p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-ink dark:text-slate-200">{currentQuestion.prompt}</p>
        ...
        {currentQuestion.options?.length ? (
              ...
              <label
                ...
                className={`flex cursor-pointer items-start gap-3 rounded-md border px-3 py-3 text-sm transition ${
                  selectedAnswer === option.key
                    ...
                    : "border-line bg-white hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100 dark:hover:bg-slate-800"
                }`}
              >
                <input
                  type="radio"
          ...
          <textarea
            className="mt-5 min-h-32 w-full rounded-md border border-line px-3 py-2 text-sm outline-none focus:border-brand focus:ring-4 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:ring-blue-950"
