...
const TopicDetail = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-ink">Subtopics</h2>
        {subtopics.length === 0 ? (
          <EmptyState title="No subtopics yet" message="Subtopics for this topic will appear here." />
        ) : (
          <div className="grid gap-3 md:grid-cols-2">
            {subtopics.map((subtopic) => (
              <div key={subtopic._id} className="rounded-md border border-line bg-white p-4">
                <h3 className="font-semibold text-ink">{subtopic.title}</h3>
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-ink">Questions</h2>
        {questions.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-md border border-line bg-white">
            {questions.map((question) => (
              <Link
                ...
                className="block border-b border-line px-4 py-4 transition last:border-b-0 hover:bg-blue-50"
              >
                ...
                <p className="mt-2 font-medium text-ink">{question.title || question.prompt}</p>
              </Link>
            ))}
          </div>
        )}
